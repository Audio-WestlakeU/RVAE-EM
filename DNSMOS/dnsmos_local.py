...
def main(args):
    ...
    with open(os.path.join(args.testset_dir, "ave_metrics_mos.txt"), "w") as f:
        print(
            "OVRL: {:.2f} ± {:.2f}".format(*mean_std(df['OVRL'].to_numpy())), file=f 
        )
        print(
            "SIG: {:.2f} ± {:.2f}".format(*mean_std(df['SIG'].to_numpy())), file=f 
        )
        print(
            "BAK: {:.2f} ± {:.2f}".format(*mean_std(df['BAK'].to_numpy())), file=f 
        )
